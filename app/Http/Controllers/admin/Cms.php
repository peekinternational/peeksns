...
class Cms extends Controller{
        ...
        if($request->isMethod('post')){
         ...
         //  dd($request->all());
            ...
            $source=$request->input('news_resource');
          $resource= substr($source,-11);
            $input['newsData'] = $request->input('newsData');
            $input['news_resource'] = $resource;
            $input['news_category'] = $request->input('news_category');
            $input['modifiedTime'] = date('Y-m-d H:i:s');
            if($pageId != '0'){
                DB::table('jcm_news')->where('newsId','=',$pageId)->update($input);
                $rMsg = 'Page Updated';
            }else{
                $input['createdTime'] = date('Y-m-d H:i:s');
                DB::table('jcm_news')->insert($input);
                $rMsg = 'Page Added';
            }
            ...
            if($rPath == 'edit'){
                ...
                $cpage = DB::table('jcm_news')->where('newsId','=',$pageId)->first();
                if(count($cpage) == 0){
    ...
    public function viewPages(Request $request){
        ...
        /* pages query*/
        $cmsPages = DB::table('jcm_news')->orderBy('newsId','desc')->paginate(25);
        /* end */
